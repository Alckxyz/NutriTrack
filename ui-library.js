...
export function renderLibraryList(container, searchInput, sortSelect, onEdit, onDelete, onToRecipe) {
    ...
    filtered.sort((a, b) => {
        ...
        if (sortVal === 'newest') {
            const timeA = Utils.toMillis(a.created_at || a.updated_at);
            const timeB = Utils.toMillis(b.created_at || b.updated_at);
            return timeB - timeA;
        }
        if (sortVal === 'edited') {
            const timeA = Utils.toMillis(a.updated_at || a.created_at);
            const timeB = Utils.toMillis(b.updated_at || b.created_at);
            return timeB - timeA;
        }
        return 0;
    });
...
export function renderRecipeLibraryList(container, query, sortSelect, onEdit, onDelete) {
    if (!container) return;
    const normalizedQuery = normalizeString(query);
